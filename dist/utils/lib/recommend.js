...
exports.sortMovie = void 0;
const getScore = (isLike, movie) => {
    ...
    if (Object.keys(likeData).length > 0 && movie) {
        ...
        const userFav = Object.keys(likeData).map((x) => Number(x));
        const isUserLike = userFav.filter((x) => movie.includes(x));
...
const sortMovie = (tags, movies) => {
    if (tags && movies) {
        ...
        return listOfMovie.sort((a, b) => b.score - a.score);
    }
    return movies;
};
exports.sortMovie = sortMovie;
